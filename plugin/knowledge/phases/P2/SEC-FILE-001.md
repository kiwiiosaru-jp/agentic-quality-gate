---
id: SEC-FILE-001
title: 対象コード・設定においてファイルアップロード（拡張子/MIME/中身の三重検証）に対応する防御機構が実装され、検証されて
phase:
- P2
gate: security/file
severity: critical
...
# SEC-FILE-001: 対象コード・設定においてファイルアップロード（拡張子/MIME/中身の三重検証）に対応する防御機構が実装され、検証されて

## 観点・確認内容

対象コード・設定においてファイルアップロード（拡張子/MIME/中身の三重検証）に対応する防御機構が実装され、検証されているか
...
5. 判定結果は `cited_knowledge: [SEC-FILE-001]` を必ず付与
